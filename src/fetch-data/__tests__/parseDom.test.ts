...
const articleWithScriptIntro = fs
  .readFileSync(path.join(__dirname, "article-intro-with-script.html"))
  .toString();
...
describe("parseDom", () => {
  ...
  test("should work with script intro", () => {
    const dom = new JSDOM(articleWithScriptIntro);
    const parsed = parseDom(dom, "article-abcd", "intro-qui-deconne-html");
    expect(parsed).toMatchSnapshot();
  });
  ...
  `("$# should parse correctly", ({ input, expected }) => {
    const dom = new JSDOM(
      `<meta property="article:modified_time" content="2022-08-02" /><main><h1>Hello</h1><div class="main-article__texte texte--editorial crayon article-texte-377828 "><h3></h3>${input}</div></main>`
    );
